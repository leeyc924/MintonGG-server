...
router.post('/upsert', async (req, res) => {
  ...
  try {
    ...
    const sql = userids ? `
      INSERT INTO game2 (play_dt, userids, play_month, play_year)
...
router.post('/remove', async (req, res) => {
  ...
  try {
    ...
    const userids = req.body['userids']?.join(', ');
    const sql = `
      UPDATE game2 SET userids = '{${userids}}'
...
router.get('/cdc', async (req, res) => {
  ...
  value = value.join(',') + ';';
  const insert = `
      INSERT INTO game2 (play_dt, userids, reg_dt, mod_dt, play_month, play_year)
      VAlUES ${value}
  `;
